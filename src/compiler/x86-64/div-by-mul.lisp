...
(defun emit-mul-shift-add-tag (result x rax signedp
  ...
  (flet ((load-values (&optional (multiplier multiplier))
           ...
           (inst mov rdx multiplier)
           (cond ((= (ldb (byte n-word-bits 0) (- multiplier))
                     increment)
                  (inst mov tmp rdx)
                  (inst neg tmp))
                 ((= (ldb (byte n-word-bits 0) (- 1 multiplier))
                     increment)
                  (inst mov tmp 1)
                  (inst sub tmp rdx))
                 (t
                  (multiple-value-bind (scale offset)
                      (round increment multiplier)
                    (unless (typep offset '(signed-byte 32))
                      (setf scale nil))
                    (case scale
                      (1
                       (aver (/= offset 0))
                       (inst lea tmp (make-ea :qword :base rdx :disp offset)))
                      ((2 4 8)
                       (inst lea tmp (make-ea :qword :index rdx :scale scale :disp offset)))
                      ((3 5 9)
                       (inst lea tmp (make-ea :qword :base rdx :index rdx :scale (1- scale)
                                                     :disp offset)))
                      (t
