...
(define-source-transform sort (&environment env sequence cmp &rest rest &key key)
  (when env
    (setf cmp (sb!xc:macroexpand cmp env)
          key (sb!xc:macroexpand key env)))
  (cond ((or (literal-lambda-form-p cmp)
             (literal-lambda-form-p key))
         (let ((cmp-fun (gensym "CMP"))
               (key-fun (gensym "KEY")))
           `(,@(if (literal-lambda-form-p cmp)
                   (let* ((x (gensym "X"))
                          (y (gensym "Y")))
                     `(flet ((,cmp-fun (,x ,y)
                                  (funcall ,cmp ,x ,y)))
                        (declare (inline ,cmp-fun))))
                   '(progn))
             ,`(,@(if (literal-lambda-form-p key)
                      (let ((x (gensym "X")))
                        `(dx-flet ((,key-fun (,x)
                                     (funcall ,key ,x)))
                           (declare (inline ,key-fun))))
                      '(progn))
                ,(if (literal-lambda-form-p key)
                     `(sort ,sequence ,(if (literal-lambda-form-p cmp)
                                           `#',cmp-fun cmp)
                            :key #',key-fun)
                     `(sort ,sequence ,(if (literal-lambda-form-p cmp)
                                           `#',cmp-fun cmp)
